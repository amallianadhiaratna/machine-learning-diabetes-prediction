...
def predict():
    ...
    if request.method == 'POST':
        if user_active!='':
            ...
            BloodPressure = float(body['BloodPressure'])
            SkinThickness = float(body['SkinThickness'])
            Insulin = float(body['Insulin'])
            BMI = float(body['BMI'])
            DiabetesPedigreeFunction = float(body['DiabetesPedigreeFunction'])
            Age = int(body['Age'])
            if SkinThickness == 0:
                SkinThickness+=29
            predictproba_test=model.predict_proba(([[
                Pregnancies,Glucose,BloodPressure,SkinThickness,Insulin, BMI,DiabetesPedigreeFunction,Age
            ]]))[0]
            print(predictproba_test)
            prediction_test=''
            result=''
            for pred in predictproba_test[:1]:
                if pred > 0.74796748:
                    prediction_test+='0'
                    result+="You don't have diabetes"
                else:
                    prediction_test+='1'
                    result+="You may have diabetes, please consult with a doctor"
            print(prediction_test)
            print(result)
            try : 
                conn = mysql.connection
                cursor = conn.cursor()
                qry_sugar='''insert into history_ (Nama, Pregnancies, Glucose, BloodPressure, SkinThickness, Insulin, BMI, DiabetesPedigreeFunction, Age, Outcome) 
                    values (%s,%s,%s,%s,%s,%s,%s,%s,%s)
                ;'''
                recordTuple = (user_active, Pregnancies, Glucose, BloodPressure, SkinThickness, Insulin, BMI, DiabetesPedigreeFunction, Age, prediction_test) 
